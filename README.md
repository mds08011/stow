...
* **Optional Post-Transcription Polish:** One-tap **Polish** button or optional auto-polish in Settings; choose **Use Raw** or **Use Polished** to copy that text and open the editable result screen. Uses a free-tier-friendly Groq chat model (`openai/gpt-oss-20b` by default) and applies Jargon Dictionary terms for project names and technical vocabulary. See [docs/polishing-prompt.md](docs/polishing-prompt.md) for the prompt design and message structure.
* **Polish Model You Can Change:** Groq retires chat models on its own schedule — it retired the one Stow used in August 2026. **Settings → Polish model** is a plain text field prefilled with the current default, so the next retirement is a paste from `console.groq.com/docs/models` rather than an app update. Leave it empty to go back to the default. Transcription is unaffected and has no such field. If a polish call fails for any reason, Stow says why in a dialog and keeps the raw transcript — it never shows partial or empty polish output as a result.
...
* **Dynamic Version Display:** Always know exactly which build you are running with a clean, dynamically generated version label (shows **v2.8.2** for this release).
...
2. Download the latest `.apk` asset (for example, `stow-app-v2.8.apk`).
...
5. Enter the desired version number (e.g., v2.8) and click the green 'Run workflow' button.

Once triggered, the automated pipeline will build a **signed** release APK, verify the signature, rename the artifact dynamically (e.g., stow-app-v2.8.apk), and attach it to a new GitHub Release page for easy downloading.
...
* **"Groq will not serve the polish model …"** means Groq has retired that model — it does this on a published schedule. Open **Settings → Polish model**, paste a current id from [console.groq.com/docs/models](https://console.groq.com/docs/models), and save. Clearing the field restores the version's default. Transcription keeps working throughout; only polish uses a chat model.
...
* If polish fails, the raw transcript is still copied and offered on the editable result screen, and a dialog explains what went wrong. Polish never returns partial, truncated, or empty text as a result — if it cannot produce a clean answer it fails and the raw transcript stands.
