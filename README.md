...
* **Upload Retry:** A dropped connection retries once automatically. If it still fails, the audio is kept and a **Retry last upload** button appears on the main screen — a lost note becomes one tap instead of a re-dictation. When Groq rate-limits you the button counts down until a retry could actually succeed, rather than letting you burn another slot.
* **Survives Being Killed:** Every recording is checkpointed the moment it stops, before the upload starts. If Stow is closed or killed mid-upload, the next launch offers the recording back rather than stranding it.
...
* **Polish Presets:** Polish behavior is driven by named, editable presets. Three ship built in:
  * **Clean prose** — the original light cleanup: removes fillers, fixes spelling/grammar/punctuation, applies only very light rephrasing. Does not rewrite, expand, or change the speaker’s voice. Produces plain text with no markers, ever.
  ...
  * **Prompt capture** — the same cleanup as Clean prose, with the result wrapped in `<transcript>` … `</transcript>` tags for pasting somewhere that wants the transcript delimited.

  Pick the preset from the compact selector next to **Polish**; the choice is remembered and is what auto-polish uses. With auto-polish on there is **no confirmation step** — you land straight on the editable result with the polished text already copied, and a **Show raw / Show polished** toggle in the result header if you want the other version. Add your own presets, edit any prompt, rename, and delete custom ones under **Settings → Polish presets…**. The three built-ins can be reset to their shipped text but never deleted.

  Any preset can wrap its output — it is a **Wrap output in `<transcript>` tags** checkbox in the preset editor, not something the prompt asks the model for. The wrapping is applied by the app after the response comes back, so it is either there or not, never occasionally. Separately, the app always strips the internal `<<<TRANSCRIPT` delimiters if the model echoes them into its answer. See [docs/polishing-prompt.md](docs/polishing-prompt.md).
...
* **Dynamic Version Display:** Always know exactly which build you are running with a clean, dynamically generated version label (shows **v2.7** for this release).
...
2. Download the latest `.apk` asset (for example, `stow-app-v2.7.apk`).
...
5. Enter the desired version number (e.g., v2.7) and click the green 'Run workflow' button.

Once triggered, the automated pipeline will build a **signed** release APK, verify the signature, rename the artifact dynamically (e.g., stow-app-v2.7.apk), and attach it to a new GitHub Release page for easy downloading.
